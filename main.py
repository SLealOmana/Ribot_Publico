...
def filetowav(dir_frases, filename):
    ...
    if (filename.endswith(".mp4")):
        os.system('ffmpeg -i {} -acodec pcm_s16le -ar 16000 {}/{}.wav'.format(
            filename, dir_frases, actual_filename))
    else:
        pass
...
async def on_message(message):
    ...
    if "wasted" in message.content:
        ...
        await bot.process_commands(message)
    if "sussy" in message.content:
        message.content = "- ssbk"
        await bot.process_commands(message)
